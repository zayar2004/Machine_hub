"""Full migration: local SQLite → Supabase Postgres."""
import os
import sys
...
def read_local():
    ...
    with app.app_context():
        ...
        data["error_images"] = [{
            ...
            "image_data": getattr(i, "image_data", None),
            "mime_type": getattr(i, "mime_type", None),
            "file_size": getattr(i, "file_size", None),
...
def write_remote(data):
    url = Path.home().joinpath("supabase_db_url.txt").read_text().strip()
    ...
    with app.app_context():
        # Clear
        print("  Clearing Supabase old data...")
...
if __name__ == "__main__":
    ...
    print()
    print("=" * 60)
    print("STEP 2: Write to Supabase")
    print("=" * 60)
    write_remote(data)
